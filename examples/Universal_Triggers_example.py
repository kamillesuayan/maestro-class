...
def get_loss_per_candidate(index, model_wrapper, batch, trigger_token_ids, cand_trigger_token_ids, snli=False):
    ...
    curr_loss = eval_with_triggers(model_wrapper, batch, trigger_token_ids, False)['loss'].cpu().detach().numpy()
...
def get_best_candidates(model_wrapper, batch, trigger_token_ids, cand_trigger_token_ids, snli=False, beam_size=1) -> List[int]:
    ...
    loss_per_candidate = get_loss_per_candidate(0, model_wrapper, batch, trigger_token_ids,
    ...
    for idx in range(1, len(trigger_token_ids)): # for all trigger tokens, skipping the 0th (we did it above)
        ...
        for cand, _ in top_candidates: # for all the beams, try all the candidates at idx
            loss_per_candidate.extend(get_loss_per_candidate(idx, model_wrapper, batch, cand,
                                                             cand_trigger_token_ids, snli))
        top_candidates = heapq.nlargest(beam_size, loss_per_candidate, key=itemgetter(1))
...
def test(model_wrapper, device,validation_sampler, num_tokens_change,vocab):
    ...
    iterator_dataloader = DataLoader(targeted_dev_data,batch_size=universal_perturb_batch_size,shuffle=True)
    ...
    for batch in iterator_dataloader:
        ...
        embedding_weight = model_wrapper.model.word_embeddings._token_embedders["tokens"].weight.cpu()
        ...
        trigger_token_ids = get_best_candidates(model_wrapper,
                                                      batch,
                                                      trigger_token_ids,
