...
def attack(iterator_dataloader: List[List[int]], vm: virtual_model, constraint: int):
    """
    ...
    for batch in iterator_dataloader:
        ...
        cand_trigger_token_ids = hotflip_attack_helper(
            averaged_grad, embedding_weight, num_candidates=40, increase_loss=True,
        )
...
def get_accuracy(vm, dev_data, trigger_token_ids, triggers=False, batch=False,) -> None:
    if batch:
        if triggers:
            print_string = ""
            ...
            outputs = vm.get_batch_output(
                dev_data["input_ids"].cpu().detach().numpy(),
                dev_data["labels"].cpu().detach().numpy(),
            )
            logits = outputs[1]
            # print(logits)
            preds = np.argmax(logits, axis=1)
...
def eval_with_triggers(
    ...
    trigger_sequence_tensor = trigger_sequence_tensor.repeat(len(batch["labels"]), 1)
    ...
    if gradient:
        data_grad = vm.get_batch_input_gradient(
            original_tokens, batch["labels"].cpu().detach().numpy()
        )
        return data_grad
    else:
        outputs = vm.get_batch_output(
...
def main():
    ...
    vm = virtual_model(url, application_name="Universal_Attack")
    ...
    perturbed = attack(iterator_dataloader, targeted_dev_data, vm, constraint=3)
    print(perturbed)
